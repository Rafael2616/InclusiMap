...
sealed interface PlaceDetailsEvent {
    data class OnUploadPlaceImages(
        val uris: List<Uri>,
        val context: Context,
        val placeId: String,
    ) : PlaceDetailsEvent

    data class OnDeletePlaceImage(
        val image: PlaceImage,
    ) : PlaceDetailsEvent

    data object OnDestroyPlaceDetails : PlaceDetailsEvent

    data class SetCurrentPlace(
        val place: AccessibleLocalMarker,
    ) : PlaceDetailsEvent

    data class SetUserAccessibilityRate(
        val rate: Int,
    ) : PlaceDetailsEvent
    ...
    data class SetIsUserCommented(
        val isCommented: Boolean,
    ) : PlaceDetailsEvent

    data class SetIsTrySendComment(
        val isTrying: Boolean,
    ) : PlaceDetailsEvent

    data class SetIsEditingPlace(
        val isEditing: Boolean,
    ) : PlaceDetailsEvent

    data class SetIsEditingComment(
        val isEditing: Boolean,
    ) : PlaceDetailsEvent

    data class OnUpdatePlaceAccessibilityResources(
        val resources: List<Resource>,
    ) : PlaceDetailsEvent

    data class GetCurrentNearestPlaceUri(
        val latLng: LatLng,
    ) : PlaceDetailsEvent
}
